  ...
  <v-row justify="center">
    <v-dialog v-model="dialog" persistent width="1024">
      <template v-slot:activator="{ props }">
        <v-btn v-if="hasPermissions('BATCH', 'createBatchJob')" color="primary" v-bind="props"
          class="mr-2"><v-icon>mdi-plus</v-icon></v-btn>
      </template>
      <v-card>
        <div class="d-flex justify-space-between align-center">
          <v-card-title>Graduation Algorithm</v-card-title>
          <v-btn @click="closeDialogAndResetForm()" class="m-4" icon="mdi-close" density="compact" rounded="sm"
                ...
                <v-stepper-window-item value="0">
                  <v-row>
                    <v-col>
                      ...
                      ]" label="Select a group" variant="outlined" hide-details>
