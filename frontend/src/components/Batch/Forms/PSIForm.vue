  ...
  <v-row justify="center">
    <v-dialog v-model="dialog" persistent width="1024">
      <template v-slot:activator="{ props }">
        <v-btn v-if="hasPermissions('BATCH', 'runPSIBatch')" color="primary" v-bind="props" class="mr-2"
          @click="setGroup('Psi')">
          ...
          <v-stepper show-actions v-model="step">
            ...
            <template v-slot:actions>
              <div class="row mx-6 mb-6">
                ...
                <v-spacer />
                <!-- Right Action Button -->
                <v-btn v-if="step < 1" @click="step++" :disabled="v$.getBatchRequest.hasAtLeastOnePSiValue.$invalid"
                  color="bcGovBlue">
                  Next
                </v-btn>
