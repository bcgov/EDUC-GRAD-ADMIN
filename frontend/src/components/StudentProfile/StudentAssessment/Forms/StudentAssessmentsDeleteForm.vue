    ...
    <v-dialog v-model="dialog" persistent max-width="80%">
      <template v-slot:activator="{ props }">
        <slot name="activator" v-bind="props">
          <v-btn v-if="!assessmentBatchDelete && hasPermissions('STUDENT', 'studentAssessmentUpdate')"
            :disabled="!hasPermissions('STUDENT', 'studentAssessmentUpdate') || studentStatus == 'MER'" v-bind="props"
      ...
      <v-card>
        ...
        <v-card-text>
          ...
          <div v-if="selectedAssessmentsWithValidations.warning.length || selectedAssessmentsWithValidations.info.length">
            <h4 class="font-weight-bold my-2">The following assessments will be deleted</h4>

            <div
              v-for="(assessment, index) in [...selectedAssessmentsWithValidations.warning, ...selectedAssessmentsWithValidations.info]"
              :key="'deletable-' + index" class="mb-3">
              <v-alert :type="assessment.validationType" border="start" variant="tonal" density="compact"
          ...
          <div v-if="selectedAssessmentsWithValidations.error.length">
            <h4 class="font-weight-bold my-2">The following assessments will NOT be deleted</h4>
            <div v-for="(assessment, index) in selectedAssessmentsWithValidations.error" :key="'error-' + index" class="mb-3">
        ...
        <v-card-actions>
          ...
          <v-spacer />
          <v-btn v-if="hasAssessmentsToDelete" color="error" variant="flat" class="text-none" density="default" :disabled="validationStep"
                 @click="confirmDelete">
            Delete Assessments<span v-if="selectedAssessmentsToDelete.length > 1">s</span>
          </v-btn>
...
export default {
  ...
  computed: {
    ...
    selectedAssessmentsWithWarnings() {
      return this.selectedAssessmentsWithValidations.filter(assessment =>
        (!assessment.validationIssues || assessment.validationIssues.length === 0) ||
        (
          assessment.validationIssues.some(issue => issue.type === 'warning') &&
          !assessment.validationIssues.some(issue => issue.type === 'error')
        )
      );
    },
    ...
    selectedAssessmentsWithValidations() {
      ...
      this.selectedAssessmentsToDelete.forEach((assessment) => {
        console.log("Assessing assessment: " + assessment);
        // add validations
  ...
  methods: {
    ...
    async confirmDelete() {
      try {
        ...
        const response = await this.studentStore.deleteStudentAssessments(request);
        console.log(response.data)

        const notDeleted = response.data
          .filter(assessment =>
            assessment.validationIssues?.some(issue => issue.validationIssueSeverityCode === "ERROR")
          )
          .map(assessment => {
