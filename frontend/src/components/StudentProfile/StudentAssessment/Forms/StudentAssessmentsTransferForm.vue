      ...
      <v-card>
                ...
                <v-row v-if="sourceStudentData.pen === targetStudentData.pen" no-gutters>
                  <v-col cols="12" class="mt-2">
                    <v-alert type="error" dense text>
                      <div class="mb-2">
                        Transferring assessments to the same student is not allowed.
                      </div>
                    </v-alert>
                  </v-col>
                </v-row>
              ...
              <v-stepper-window-item value="3">
                <div style="max-height: 60vh; overflow-y: auto; padding-right: 0.5rem;">
                  <v-expansion-panels multiple>
                    ...
                    <v-expansion-panel v-for="(assessment, index) in transferStudentAssessmentResultsMessages"
                      :key="index" elevation="0" rounded="0" style="background-color: transparent; border:none">

                      <v-alert
                        :type="assessment.validationIssues.some(i => i.validationIssueSeverityCode === 'ERROR') ? 'error' : 'success'"
                        border="start" variant="tonal" density="compact" class="my-1">

                        <template #prepend>
                          <v-icon
                            :color="assessment.validationIssues.some(i => i.validationIssueSeverityCode === 'ERROR') ? 'error' : 'success'"
                            style="margin-top: 0.94rem;">
                            {{
                              assessment.validationIssues.some(i => i.validationIssueSeverityCode === 'ERROR')
                                ? 'mdi-close-circle'
                                : 'mdi-check-circle'
                            }}
                          </v-icon>
                        ...
                        <v-expansion-panel-title class="d-flex align-center justify-space-between">
                          <div class="d-flex align-center flex-grow-1">
                            ...
                            <span>
                              ...
                              {{ assessment.originalAssessment.assessmentDetails.assessmentCode }}
                              {{ assessment.originalAssessment.assessmentDetails.assessmentLevel }} –
                            ...
                            <span class="text-caption text-grey-darken-1">
                              {{
                                (() => {
                                  const errors = assessment.validationIssues.filter(
                                    (i) => i.validationIssueSeverityCode === "ERROR"
                                  ).length;
                                  const warnings = assessment.validationIssues.filter(
                        ...
                        <v-expansion-panel-text>
                          <div v-if="assessment.validationIssues.length">
                            <div v-for="(issue, i) in assessment.validationIssues" :key="i"
                  ...
                  <v-alert v-if="assessmentsToTransfer.length > 0" type="info" class="mb-4" border="start" elevation="2"
                    variant="tonal">
                    <div class="mb-2">
                      You are about to transfer the following assessments to student
                      <strong>{{ targetStudentData.pen }} - {{ targetStudentData.legalLastName }}, {{
                        targetStudentData.legalFirstName }}</strong>:
                    </div>
                    <v-row no-gutters v-for="assessment in assessmentsToTransfer"
                      :key="assessment.assessmentID + assessment.sessionDate" </v-row>
                  </v-alert>
                </div>
              </v-stepper-window-item>
            </v-stepper-window>
        ...
        <v-card-actions>
          ...
          <v-btn v-else-if="step <= 2" @click="step--" color="bcGovBlue" variant="outlined"
            :disabled="validationStep || (transferStudentAssessmentResultsMessages && transferStudentAssessmentResultsMessages.length > 0)">Back</v-btn>
          <v-spacer />
          <v-btn v-if="step === 0 || (step === 1 && assessmentsToTransfer.length > 0)" @click="step++" color="bcGovBlue"
            variant="outlined"
            :disabled="validationStep || (step === 1 && assessmentsToTransfer.length === 0) || (step === 0 && v$.$invalid) || (sourceStudentData.pen === targetStudentData.pen)">Next</v-btn>
          <v-btn v-else-if="step == 2 && assessmentsToTransfer.length > 0" :disabled="validationStep" color="error"
            variant="flat" class="text-none" @click="submitForm">
...
export default {
    ...
    targetStudentData: {
      ...
      async handler(newTargetStudentData) {
        if (newTargetStudentData && newTargetStudentData.studentID) {
          ...
          let response = await this.getStudentAssessments(newTargetStudentData.studentID);
  ...
  methods: {
    ...
    async submitForm() {
      ...
      const assessmentWithoutAssessmentDetailsAndUserInfo = this.assessmentsToTransfer.map(({ assessmentDetails, relatedAssessmentDetails, createUser, createDate, updateUser, updateDate, ...rest }) => ({ ...rest }));
      const transferStudentAssessmentsRequestBody = toRaw(assessmentWithoutAssessmentDetailsAndUserInfo);
      try {
        await this.transferStudentAssessments(this.sourceStudentData.studentID, this.targetStudentData.studentID, transferStudentAssessmentsRequestBody).then((response) => {
        ...
        this.snackbarStore.showSnackbar("Failed to transfer student assessments", "error", 10000);
      }
    },
  },
};
