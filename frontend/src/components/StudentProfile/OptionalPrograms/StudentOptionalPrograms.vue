<template>
  <div>
    <v-alert type="info" variant="tonal" border="start" class="width-fit-content">This student is on the
    ...
    <div>
      ...
      <v-data-table v-if="studentOptionalPrograms" :items="studentOptionalPrograms"
        :headers="studentOptionalProgramsHeaders" :items-per-page="'-1'" hide-default-footer>
        ...
        <template v-slot:item.optionalReqMet="{ item }">
          <div v-if="item.studentOptionalProgramData">
            ...
            " :items="item.studentOptionalProgramData.optionalRequirementsMet" :headers="requirementsMetHeaders"
            ...
            <v-data-table :items="filteredGradReqCourses(
              item.studentOptionalProgramData.optionalStudentCourses
                .studentCourseList
            )
              " :headers="studentCourseListHeaders" v-if="
                item.studentOptionalProgramData.optionalStudentCourses &&
              ...
              <template v-slot:item.gradReqMetDetail="{ item }">
                <div class="p-2">
                  <strong>{{ item.gradReqMetDetail }}</strong><br />
            ...
            <v-data-table :items="filteredGradReqAssessments(
        ...
        <template v-slot:item.optionalNonGradReasons="{ item }">
          ...
          " id="optionalNonGradReasons">
            <li v-for="optionalNonGradReason in item.studentOptionalProgramData
              .optionalNonGradReasons" :key="optionalNonGradReason.rule">
              <strong>{{ optionalNonGradReason?.rule }} -
        ...
        <template v-slot:item.actions="{ item }">
          <v-dialog v-model="deleteDialog[item.id]" v-if="
            item?.optionalProgramCode !== 'CP'
          ">
            <template v-slot:activator="{ props }">
              <v-btn v-if="hasPermissions('STUDENT', 'optionalProgramUpdate')" v-bind="props" color="error"
                icon="mdi-delete-forever" density="compact" variant="text"></v-btn>
            </template>
            <v-card title="Delete Student Optional Program" max-width="500px" class="mx-auto">
              ...
              <v-alert v-else-if="v$.ifStudentStatusDeceased.$invalid" type="warning" variant="tonal" border="start"
                class="mx-4 my-4">
                <p><strong>WARNING</strong></p>
                <p>{{ v$.ifStudentStatusDeceased.$message }}</p>
              </v-alert>
              <v-alert v-else-if="v$.ifStudentStatusTerminated.$invalid" type="warning" variant="tonal" border="start"
                class="mx-4 my-4">
                <p><strong>WARNING</strong></p>
              ...
              " type="warning" variant="tonal" border="start" class="mx-4 my-4">
                <p><strong>WARNING</strong></p>
                <p>
                  You are about to delete the
                  <strong>{{ item.optionalProgramName }} ({{ item.optionalProgramCode }})</strong>
                  Optional Program from this student
                </p>
              </v-alert>

              <v-alert v-else-if="v$.ifStudentStatusMerged.$invalid" type="error" variant="tonal" border="start"
                class="mx-4 my-4">
                <p><strong>ERROR</strong></p>
                <p>{{ v$.ifStudentStatusMerged.$message }}</p>
              </v-alert>
              <v-alert v-else-if="v$.ifProgramComplete.$invalid" type="error" variant="tonal" border="start"
                class="mx-4 my-4">
                <p><strong>ERROR</strong></p>
              ...
              <v-card-actions class="mx-2 mb-2">
                <v-btn color="error" variant="outlined" class="text-none" density="default"
                  @click="deleteDialog[item.id] = false">Cancel</v-btn>
                <v-spacer />
                <v-btn :disabled="v$.ifStudentStatusMerged.$invalid ||
                  v$.ifProgramComplete.$invalid
                  " color="error" variant="flat" class="text-none" density="default"
                  @click="deleteStudentOptionalProgram(item)">Delete Optional Program</v-btn>
              </v-card-actions>
            </v-card>
          </v-dialog>
        </template>
          ...
          <tr>
            <td :colspan="columns.length">
              <v-card class="my-3 mx-0">
                <v-data-table :items="studentCareerPrograms" :headers="studentCareerProgramsHeaders"
                  ...
                  <template v-slot:item.actions="{ item }">
                    <v-dialog v-model="deleteDialog[item.id]">
                      <template v-slot:activator="{ props }">
                        ...
                        " v-bind="props" color="error" icon="mdi-delete-forever" density="compact"
                          variant="text"></v-btn>
                      </template>
                      <v-card title="Delete Student Career Program" max-width="500px" class="mx-auto">
                        <v-divider color="error" class="mt-0 mx-4" />
                        <!-- START Career Program Delete Warnings and Errors -->
                        <v-alert v-if="v$.ifStudentStatusArchived.$invalid" type="warning" variant="tonal"
                          border="start" class="mx-4 my-4">
                          <p><strong>WARNING</strong></p>
                          <p>{{ v$.ifStudentStatusArchived.$message }}</p>
                        </v-alert>
                        <v-alert v-else-if="v$.ifStudentStatusDeceased.$invalid" type="warning" variant="tonal"
                          border="start" class="mx-4 my-4">
                          <p><strong>WARNING</strong></p>
                          <p>{{ v$.ifStudentStatusDeceased.$message }}</p>
                        </v-alert>
                        <v-alert v-else-if="v$.ifStudentStatusTerminated.$invalid" type="warning" variant="tonal"
                          border="start" class="mx-4 my-4">
                          <p><strong>WARNING</strong></p>
                          <p>{{ v$.ifStudentStatusTerminated.$message }}</p>
                        </v-alert>
                        ...
                        <v-alert v-else-if="v$.ifStudentStatusMerged.$invalid" type="error" variant="tonal"
                          border="start" class="mx-4 my-4">
                          <p><strong>ERROR</strong></p>
                          <p>{{ v$.ifStudentStatusMerged.$message }}</p>
                        </v-alert>
                        ...
                        <v-card-actions class="mx-2 mb-2">
                          ...
                          <v-btn :disabled="v$.ifStudentStatusMerged.$invalid ||
                            v$.ifProgramComplete.$invalid
                            " color="error" variant="flat" class="text-none" density="default"
                            @click="deleteStudentCareerProgram(item)">Delete Career Program</v-btn>
