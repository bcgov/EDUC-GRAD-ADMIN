...
export default {
  ...
  mounted() {
    ...
    if (
      !this.course?.credits &&
      this.creditsAvailableForCourseSession.length > 0
    ) {
    ...
    return {
      course: {
        ...
        credits: {
          ...
          creditsMustBe4ifStudentIsOnProgram1995AndCourseisB:
            helpers.withMessage(
              ...
              function (value) {
                const is1996Program =
                  this.studentProgram === "1996-EN" ||
                  this.studentProgram === "1996-FR";
          ...
          creditsMustBeAtLeast2ifStudentIsOnProgram1995AndCourseisForA:
            helpers.withMessage(
              ...
              function (value) {
                const is1996Program =
                  ...
                  this.studentProgram === "1996-FR";
                const isAorF =
                  this.course.fineArtsAppliedSkills === "A" ||
                  this.course.fineArtsAppliedSkills === "F";
  ...
  watch: {
    ...
    "course.interimPercent"(newVal) {
      if (newVal) {
        this.course.interimLetterGrade =
          this.filteredInterimLetterGrades[0] ?? "";
      } else {
        this.course.interimLetterGrade = "";
      }
    },
    ...
    "course.fineArtsAppliedSkills"(newVal) {
      ...
      if (
        newVal &&
        (courseType === "Locally Developed" ||
          courseType === "Career Program") &&
        trimmedProgram !== "1996-EN" &&
        trimmedProgram !== "1996-PF"
      ) {
        this.warnings.push(
          "Flag is only applicable for this course type if student is on the 1995 program."
        );
      }
    },
  },
  computed: {
    ...
    creditsAvailableForCourseSession() {
      if (
        !this.course.courseSession ||
        !Array.isArray(this.course.courseDetails.courseAllowableCredit)
      )
        return [];
      ...
      const courseCompletionEndDate = new Date(
        this.course.courseDetails.completionEndDate
      );
      if (
        courseEndDate &&
        courseCompletionEndDate &&
        sessionDate > courseEndDate &&
        sessionDate < courseCompletionEndDate
      ) {
        sessionDate = courseEndDate;
      }
      return this.course.courseDetails.courseAllowableCredit
        .filter((credit) => {
          ...
          const end = credit.endDate
            ? new Date(credit.endDate)
            : new Date("9999-12-31");
  ...
  methods: {
    updateWarnings() {
      ...
      if ((this.course?.courseDetails.courseCode).startsWith("Q")) {
        this.warnings.push(
          "Only use Q code if student was on Adult program at time of course completion or if course is marked as Equivalency."
        );
      }
      ...
      if (
        this.course.courseSession < this.currentReportingPeriod &&
        !this.course.finalPercent &&
        (this.course.finalLetterGrade === "" ||
          this.course.finalLetterGrade === null ||
          this.course.finalLetterGrade === undefined)
      ) {
        this.warnings.push(
    ...
    getGradesForPercent(percent) {
      const isGTorGTF =
        this.course.courseDetails.courseCode === "GT" ||
        this.course.courseDetails.courseCode === "GTF";
      ...
      if (isGTorGTF) {
        ...
        allAllowableLetterGradesForCourse = this.allLetterGrades.filter(
          (grade) => grade.grade === "RM"
        );
      } else {
        ...
        allAllowableLetterGradesForCourse = this.allLetterGrades.filter(
          (grade) => {
            ...
            const expiryDate = grade.expiryDate
      ...
      return allAllowableLetterGradesForCourse
        .filter(
          (grade) =>
            ...
            grade.percentRangeHigh !== null &&
            grade.percentRangeLow <= numPercent &&
            numPercent <= grade.percentRangeHigh
        )
        .map((grade) => grade.grade);
    },
  },
