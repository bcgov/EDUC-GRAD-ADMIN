<template>
  <v-col cols="12">
    <v-row style="font-size: 1rem">
      <v-col>
        <strong>
          {{ processedAssessment.assessmentType?.label || processedAssessment.assessmentTypeCode }}
          {{ processedAssessment.sessionDate }}</strong> ({{ processedAssessment.assessmentDetails.label }})
        {{ processedAssessment.assessmentStudentID }}
      </v-col>
    </v-row>
    ...
    <v-row no-gutters style="font-size: 1rem">
      <v-col class="mr-2" cols="auto">
        ...
        <span v-if="processedAssessment.proficiencyScore">{{ processedAssessment.proficiencyScore }}</span>
      ...
      <v-col class="mr-2" cols="auto">
        ...
        <span v-if="processedAssessment.provincialSpecialCaseCode">
          {{ getProvincialSpecialCaseDisplayName(processedAssessment.provincialSpecialCaseCode) }}
        </span>
        <span v-else><i>null</i></span>
      </v-col>
      ...
      <v-col class="mr-2" cols="auto">
        ...
        <span v-if="processedAssessment.wroteFlag">{{ processedAssessment.wroteFlag ? "Y" : "N" }}</span>
        <span v-else><i>null</i></span>
      </v-col>
      ...
      <v-col class="mr-2" cols="auto">
        ...
        <span v-if="processedAssessment.assessmentCenterSchoolID">
          {{ getAssessmentCenterSchoolDisplayName(processedAssessment.assessmentCenterSchoolID) }}
        </span>
        <span v-else><i>null</i></span>
      </v-col>
    </v-row>
...
export default {
  ...
  computed: {
    ...
    processedAssessment() {
      ...
      return {
        ...this.assessment,
        assessmentDetails: toRaw(this.assessmentTypeCodesMap.get(this.assessment.assessmentTypeCode)) || null,
