...
export default {
  ...
  methods: {
    ...
    removeCourseInfo(item) {
      this.courseReconciliation.info = this.courseReconciliation.info.filter(
        (course) =>
          !(
            course.source.courseID === item.courseID &&
            course.source.courseSession === item.courseSession
          )
      );
