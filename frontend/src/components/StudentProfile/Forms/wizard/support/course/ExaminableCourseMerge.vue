    ...
    <div v-for="(value, key) in courseReconciliation" :key="key">
        <v-row no-gutters v-if="value.length >= 0">
            <v-alert :type="getProperty(key, 'type')" :icon="getProperty(key, 'icon')" class="mb-4" border="start"
                ...
                <div class="mb-2">
                    {{ examinableCoursesToMerge[key].length }} / {{ value.length }} {{ getProperty(key, 'headerSuffix') }}
                </div>
            </v-alert>
            <v-data-table v-if="value.length > 0" v-model="examinableCoursesToMerge[key]" :items="value"
...
export default {
    ...
    methods: {
        ...
        isRowSelected(key, course) {
            if (key === 'info' || key === 'conflicts' && course && this.examinableCoursesToMerge[key].length > 0) {
                return this.examinableCoursesToMerge[key].some(entry => entry.source.courseID === course?.courseID && entry.source.courseSession === course?.courseSession);
            }
            return false;
        },
