<template>
  <div v-for="(value, key) in assessmentReconciliation" :key="key">
    <v-row no-gutters v-if="value.length >= 0">
      ...
      >
        <div class="mb-2">
          {{ value.length }} / {{ totalNoOfAssessments }}
      ...
      <v-data-table
        ...
        class="mb-4"
        hide-default-footer
        :headers="assessmentTransferHeaders(key)"
        :items-per-page="-1"
      >
        ...
        <template v-slot:item.target="{ item }">
          <div v-if="item.target" style="max-width: 98%; font-size: 0.875rem">
        ...
        <template v-if="key === 'info'" v-slot:item.delete="{ item }">
          <v-btn
            @click="removeAssessmentInfo(item.source)"
            color="error"
    ...
    totalNoOfAssessments() {
      return (
        this.assessmentReconciliation.info.length +
          ...
          this.assessmentReconciliation.errors.length || 0
      );
    },
  },
  props: {
  ...
  methods: {
    ...
    removeAssessmentInfo(item) {
      this.assessmentReconciliation.info =
        this.assessmentReconciliation.info.filter(
          (assessment) =>
            !(
              assessment.source.assessmentID === item.assessmentID &&
              assessment.source.assessmentSession === item.assessmentSession
            )
        );
      this.removeAssessmentFromTransfer(
        item.assessmentID,
        item.assessmentSession
      );
    },
