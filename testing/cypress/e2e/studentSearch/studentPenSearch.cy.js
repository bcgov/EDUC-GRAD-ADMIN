...
describe('Student Search', () => {
  ...
  context('with PEN', () => {
    ...
    it('Courses', () => {
      ...
      cy.get(selectors.studentSearch.coursesWindow).should('not.have.css', 'display', 'none')
      cy.doesHaveItemsInWindowTable(selectors.studentSearch.coursesWindow).then((exist) => {
        if (exist) {
          cy.get(selectors.studentSearch.coursesWindow).find(selectors.studentSearch.rows).its('length').should('be.gt', 0)
        } else {
          cy.get(selectors.studentSearch.coursesWindow).find(selectors.studentSearch.noRow).its('length').should('eq', 1).and('contain.text', 'No data available')
        }
      })
    })    
