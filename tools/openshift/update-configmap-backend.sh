...
oc create -n "$OPENSHIFT_NAMESPACE" configmap "$APP_NAME"-backend-config-map \
  ...
  --from-literal=SHOW_LEGACY=$showLegacy \
  ...
  --from-literal=PSI_SELECTION_ENDPOINT="http://psi-selection-api-master.$STUDENT_ASSESSMENT_NAMESPACE-$ENV.svc.cluster.local:8080" \
