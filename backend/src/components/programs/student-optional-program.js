...
async function getStudentOptionalProgramPaginated(req, res) {
  try {
    let searchArray = [];
    ...
    let data = await getCommonServiceData(
      ...
      }/grad/student/optionalprogram/search/pagination`,
      params
    );
    ...
    await logApiError(e, 'Error getting student optional program paginated list');
    if (e.data?.message) {
