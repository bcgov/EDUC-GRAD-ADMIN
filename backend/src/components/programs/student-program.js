...
const { createMoreFiltersSearchCriteria } = require('../programs/studentProgramFilters');
const API_BASE_ROUTE = '/api/v1/student';

async function getStudentProgramPaginated(req, res) {
  try {
    let searchArray = [];
    ...
    await logApiError(e, 'Error getting student program paginated list');
    if (e.data?.message) {
...
async function getProgramStudentSearchReport(req, res) {
  try {
    const search = [];
    if (req.query?.searchParams) {
